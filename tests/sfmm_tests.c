...
//}
Test(sfmm_student_suite,sf_malloc_size_zero_request,.timeout=TEST_TIMEOUT){
	//malloc size 0
	void *x=sf_malloc(0);

	cr_assert(x==NULL,"The malloc pointer should be NULL for size 0 allocation");
}

Test(sfmm_student_suite,sf_realloc_size_zero,.timeout=TEST_TIMEOUT){
	//realloc size 0
	void *x=sf_malloc(4);
	...
	cr_assert(y==NULL,"Realloc request of size 0 should free the block");
}

Test(sfmm_student_suite,one_char_malloc_request,.timeout=TEST_TIMEOUT){
	//one char malloc request
	void *x=sf_malloc(sizeof(char));

	cr_assert((((sf_block*)((char*)x-16))->header & ~0xf & 0xffff) == 32,"1 byte request should get a 32 byte block");
}

Test(sfmm_student_suite,malloc_2_pages_one_go,.timeout=TEST_TIMEOUT){
	void *x= sf_malloc(4097);

	cr_assert((unsigned long)(sf_mem_end()-x+40)==8184,"Malloc greater than 4097 requests two pages %lu",(unsigned long)(sf_mem_end()-x+40));
}

Test(sfmm_student_suite,realloc_einval,.timeout=TEST_TIMEOUT){
	void *x= sf_malloc(4097);
	void *y=sf_realloc(x+1,10);
	cr_assert(y==NULL,"Realloc for invalid request should return NULL");
}
